                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <form action="{{ route('schedules.index') }}" method="GET" class="form-inline">
                                    <input type="text" name="search" class="form-control mr-2" placeholder="Cari schedule..." value="{{ request('search') }}">
                                    <button type="submit" class="btn btn-outline-primary">Cari</button>
                                </form>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="scheduleTable" class="table table-striped table-hover" style="min-width: 845px;">
                                        ...
                                        <tbody class="text-dark">
                                            ...
                                            @foreach ($schedules as $schedule)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="#scheduleMapModal{{ $schedule->id }}" data-toggle="modal" class="btn btn-outline-info btn-sm">Lihat Peta</a>
                                                    </td>
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#scheduleDetailModal{{ $schedule->id }}">
                                                                    <i class="bx bx-info-circle"></i> Lihat Detail
                                                                </a>
                                                                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#scheduleEditModal{{ $schedule->id }}">
                                                                    <i class="bx bx-edit-alt"></i> Edit
                                                                </a>
                                                                <form action="/delete-schedule/{{ $schedule->id }}" method="POST" class="d-inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit" class="dropdown-item" onclick="return confirm('Apakah Anda yakin ingin menghapus schedule ini?')">
                                    ...
                                    @foreach ($schedules as $schedule)
                                        ...
                                        <div class="modal fade" id="scheduleMapModal{{ $schedule->id }}" tabindex="-1" role="dialog" aria-labelledby="scheduleMapModalLabel{{ $schedule->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-info text-white">
                                                        <h5 class="modal-title" id="scheduleMapModalLabel{{ $schedule->id }}">Peta Tempat: {{ $schedule->venue_name }}</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body p-3">
                                                        <iframe src="{{ $schedule->venue_map }}" width="100%" height="400" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
                                                    </div>
                                        ...
                                        <div class="modal fade" id="scheduleDetailModal{{ $schedule->id }}" tabindex="-1" role="dialog" aria-labelledby="scheduleDetailModalLabel{{ $schedule->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-primary text-white">
                                                        <h5 class="modal-title" id="scheduleDetailModalLabel{{ $schedule->id }}">Detail Pertandingan: {{ $schedule->name }}</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body p-3">
                                                        <p><strong>Nama Pertandingan:</strong> {{ $schedule->name }}</p>
                                                        <p><strong>Tanggal:</strong> {{ $schedule->date }}</p>
                                                        <p><strong>Kategori Olahraga:</strong> {{ $schedule->sport_category }}</p>
                                        ...
                                        <div class="modal fade" id="scheduleEditModal{{ $schedule->id }}" tabindex="-1" role="dialog" aria-labelledby="scheduleEditModalLabel{{ $schedule->id }}" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header bg-warning text-dark">
                                                        <h5 class="modal-title" id="scheduleEditModalLabel{{ $schedule->id }}">Edit Schedule: {{ $schedule->name }}</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body p-3">
                                                        <form action="/edit-schedule/{{ $schedule->id }}" method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            ...
                                                            <div class="form-group mb-3">
                                                                ...
                                                                <input type="text" class="form-control" id="sport_category" name="sport_category" value="{{ $schedule->sport_category }}" required>
                                                            </div>
                                                            <div class="form-group mb-3">
                                                                <label for="venue_name">Nama Tempat</label>
                                                                <input type="text" class="form-control" id="venue_name" name="venue_name" value="{{ $schedule->venue_name }}" required>
                                                            </div>
                                                            <div class="form-group mb-3">
